...
RUN cd website && \
    ...
    mkdir -p node_modules/@davincios/drizzle node_modules/@davincios/graphql node_modules/@davincios/translations && \
    echo "  Created stub directories (drizzle, graphql, translations)" && \
    ...
    echo "  (graphql stub will be generated by create-graphql-stub.js after scripts/ is copied)" && \
...
# Install the real @davincios/ui package from vendored tgz
# (replaces the stub UI components with the actual Payload CMS UI rebranded to DaVinciOS)
RUN mkdir -p website/node_modules/@davincios/ui && \
    tar -xzf scripts/vendor/davincios-ui.tgz -C website/node_modules/@davincios/ui --strip-components=1 2>/dev/null && \
    echo "  Installed @davincios/ui from vendored package ($(ls website/node_modules/@davincios/ui/dist/ 2>/dev/null | wc -l) dist entries)"

# Run stub generators for other @davincios/* packages
RUN cd website && node ../scripts/create-drizzle-stub.js
RUN cd website && node ../scripts/create-graphql-stub.js
