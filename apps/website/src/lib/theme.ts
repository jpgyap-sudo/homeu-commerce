/**
 * Theme / homepage-section data layer.
 *
 * The homepage is an ordered list of typed sections stored in
 * `homepage_sections`. Each section has a `type` and a free-form `config`
 * (JSONB). Adding a new section type = add it to SECTION_TYPES + a renderer
 * (storefront) + an editor schema (admin). Nothing else changes.
 */
