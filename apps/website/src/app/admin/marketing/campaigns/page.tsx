 ...
 * table of campaigns from the `campaigns` table with joined segment/template
 * names and recipient analytics.
 *
...
const ALLOWED_SORT = [
  ...
  'scheduled_at DESC', 'scheduled_at ASC',
] as const
...
function PaginationLink({
  ...
  const href = `/admin/marketing/campaigns?${params.toString()}`
...
export default async function AdminCampaignsListPage({ searchParams }: ListPageProps) {
  // Auth check
  ...
  if (search) {
    idx++
    conditions.push(`(c.name ILIKE $${idx} OR c.subject ILIKE $${idx})`)
    values.push(`%${search}%`)
  }
  ...
  try {
    ...
    campaigns = campaignRes.rows as CampaignRow[]
  } catch {
    campaigns = []
  }
  ...
  return (
    <main style={{ maxWidth: 1200, margin: '40px auto', padding: '0 24px', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{
        ...
        <div>
          <h1 style={{ margin: 0, fontSize: 24, fontWeight: 700, color: '#151a17' }}>Campaigns</h1>
          <p style={{ margin: '4px 0 0', color: '#667168', fontSize: 14 }}>
            {total} campaign{total !== 1 ? 's' : ''} in database
          </p>
        </div>
        <Link
      ...
      <form
        method="GET"
        action="/admin/marketing/campaigns"
        style={{
      ...
      {campaigns.length === 0 ? (
        <div style={{
          ...
          <p style={{ fontSize: 18, color: '#667168', marginBottom: 8 }}>No campaigns found</p>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
              <thead>
                <tr style={{ borderBottom: '1px solid #d9e0d7' }}>
                  ...
                  <th style={{ textAlign: 'left', padding: '12px 16px', fontSize: 11, fontWeight: 600, color: '#667168', textTransform: 'uppercase', letterSpacing: '0.05em', background: '#f7f9f6' }}>Subject</th>
                  <th style={{ textAlign: 'left', padding: '12px 16px', fontSize: 11, fontWeight: 600, color: '#667168', textTransform: 'uppercase', letterSpacing: '0.05em', background: '#f7f9f6' }}>Segment</th>
                  <th style={{ textAlign: 'left', padding: '12px 16px', fontSize: 11, fontWeight: 600, color: '#667168', textTransform: 'uppercase', letterSpacing: '0.05em', background: '#f7f9f6' }}>Template</th>
                  <SortableHeader label="Status" sortKey="status ASC" currentSort={sort} />
                  <th style={{ textAlign: 'left', padding: '12px 16px', fontSize: 11, fontWeight: 600, color: '#667168', textTransform: 'uppercase', letterSpacing: '0.05em', background: '#f7f9f6' }}>Recipients</th>
                  <SortableHeader label="Scheduled" sortKey="scheduled_at DESC" currentSort={sort} />
              ...
              <tbody>
                {campaigns.map(campaign => (
                  <tr key={campaign.id} style={{ borderBottom: '1px solid #eef1ed' }}>
                    ...
                    <td style={{ padding: '12px 16px', color: '#667168', fontSize: 13 }}>{campaign.subject}</td>
                    <td style={{ padding: '12px 16px', fontSize: 13 }}>{campaign.segment_name || '—'}</td>
                    <td style={{ padding: '12px 16px', fontSize: 13 }}>{campaign.template_name || '—'}</td>
                    <td style={{ padding: '12px 16px' }}>
                      <span>{campaignStatusBadge(campaign.status)}</span>
                    </td>
                    <td style={{ padding: '12px 16px', fontSize: 12, color: '#667168' }}>
                      {campaign.sent_count} / {campaign.recipients} sent
                    </td>
                    <td style={{ padding: '12px 16px', fontSize: 12, color: '#667168' }}>
                      {campaign.scheduled_at ? formatDate(campaign.scheduled_at) : '—'}
                    </td>
                    <td style={{ padding: '12px 16px', fontSize: 12, color: '#667168' }}>{formatDate(campaign.created_at)}</td>
