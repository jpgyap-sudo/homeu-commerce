 ...
 * table of email templates from the `email_templates` table.
 *
...
function PaginationLink({
  ...
  const href = `/admin/marketing/templates?${params.toString()}`
...
export default async function AdminTemplatesListPage({ searchParams }: ListPageProps) {
  // Auth check
  ...
  try {
    ...
    templates = templateRes.rows as TemplateRow[]
  } catch {
    templates = []
  }
  ...
  return (
    <main style={{ maxWidth: 1200, margin: '40px auto', padding: '0 24px', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{
        ...
        <div>
          <h1 style={{ margin: 0, fontSize: 24, fontWeight: 700, color: '#151a17' }}>Email Templates</h1>
          <p style={{ margin: '4px 0 0', color: '#667168', fontSize: 14 }}>
            {total} template{total !== 1 ? 's' : ''} in database
          </p>
        </div>
        <Link
      ...
      <form
        method="GET"
        action="/admin/marketing/templates"
        style={{
      ...
      {templates.length === 0 ? (
        <div style={{
          ...
          <p style={{ fontSize: 18, color: '#667168', marginBottom: 8 }}>No templates found</p>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
              <thead>
                <tr style={{ borderBottom: '1px solid #d9e0d7' }}>
                  ...
                  <SortableHeader label="Subject" sortKey="subject ASC" currentSort={sort} />
                  <th style={{ textAlign: 'left', padding: '12px 16px', fontSize: 11, fontWeight: 600, color: '#667168', textTransform: 'uppercase', letterSpacing: '0.05em', background: '#f7f9f6' }}>Preheader</th>
              ...
              <tbody>
                {templates.map(template => (
                  <tr key={template.id} style={{ borderBottom: '1px solid #eef1ed' }}>
                    ...
                    <td style={{ padding: '12px 16px', color: '#151a17', fontSize: 13 }}>{template.subject}</td>
                    <td style={{ padding: '12px 16px', color: '#667168', fontSize: 13, maxWidth: 300, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {template.preheader || '—'}
                    </td>
                    <td style={{ padding: '12px 16px', fontSize: 12, color: '#667168' }}>{formatDate(template.created_at)}</td>
