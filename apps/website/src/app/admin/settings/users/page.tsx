...
export default function UsersPage() {
  ...
  const saveUser = async () => {
    ...
    const body: any = editing ? { id: editing.id, name: form.name, role: form.role, tabs: form.tabs } : form
    if (!editing && form.password) (body as any).password = form.password
    ...
    if (r.ok) {
      ...
      const d = await r.json(); setMsg('Error: ' + (d.error || 'failed'))
    }
  }

  const tabs = (user: User) => {
    try { return typeof user.tab_permissions === 'string' ? JSON.parse(user.tab_permissions) : (user.tab_permissions || ['*']) } catch { return ['*'] }
  }
  ...
  return (
    <div>
      <header className="luxe-page-header">
        ...
        <button onClick={() => { setCreating(true); setEditing(null); setForm({ email:'', name:'', role:'editor', password:'', tabs:['*'] }) }} className="luxe-btn luxe-btn-gold luxe-btn-sm" style={{ marginTop: 'var(--space-4)' }}>
          + New User
        </button>
      </header>
              ...
              <tr key={user.id}>
                ...
                <td style={{ maxWidth: 200 }}>
                  <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                    {tabs(user).includes('*') ? <span className="luxe-badge info" style={{ fontSize: 9 }}>All</span> :
                ...
                <td>
                  ...
                  }} className="luxe-btn luxe-btn-ghost luxe-btn-sm" style={{ fontSize: 10 }}>✏️ Edit</button>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 999, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => { setEditing(null); setCreating(false) }}>
          <div className="luxe-card" style={{ width: 500, maxWidth: '90vw', maxHeight: '85vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}>
            <div className="luxe-card-header">
              <h2 className="luxe-card-title">{editing ? '✏️ Edit User' : '➕ New User'}</h2>
            </div>
