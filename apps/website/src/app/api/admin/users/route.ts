...
export async function GET() {
  ...
  try {
    const { rows } = await query('SELECT id, email, name, role, status, tab_permissions, created_at FROM customers ORDER BY role, name')
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const { id, name, role, status, tabs, password } = body
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { rows } = await query(
      ...
      [email, name || email.split('@')[0], role || 'editor', hash, JSON.stringify(safeTabs)]
    )
    return NextResponse.json({ user: rows[0] })
  } catch (err: any) { return NextResponse.json({ error: err.message }, { status: 500 }) }
}
