...
/**
 * Send an email via the configured provider (SMTP or SES).
 * SES support requires @aws-sdk/client-sesv2 (optional peer dependency).
 */
async function sendEmail({ to, subject, html, text }) {
  if (process.env.SEND_PROVIDER === 'ses') {
...
async function sendViaSes({ to, subject, html, text }) {
  // Dynamic import — only required if SEND_PROVIDER=ses
