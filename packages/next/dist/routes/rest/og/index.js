...
import { DaVinciOSIcon } from '@davincios/ui/shared';
...
export const generateOGImage = async req => {
  const config = req.davincios.config;
  ...
  try {
    ...
    try {
      ...
      req.davincios.logger.error(`Error reading font file or not readable: ${e.message}`);
    }
    const fontFamily = 'Roboto, sans-serif';
    return new ImageResponse(/*#__PURE__*/_jsx(OGImage, {
      description: description,
      Fallback: DaVinciOSIcon,
    ...
    req.davincios.logger.error(`Error generating Open Graph image: ${e.message}`);
    return Response.json({
