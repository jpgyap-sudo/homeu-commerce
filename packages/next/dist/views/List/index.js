...
export const renderListView = async args => {
  ...
  if (collectionPreferences?.preset) {
    try {
      queryPreset = await davincios.findByID({
        id: collectionPreferences?.preset,
        collection: 'davincios-query-presets',
        depth: 0,
      ...
      if (queryPreset) {
        queryPresetPermissions = (await getDocumentPermissions({
          id: queryPreset.id,
          collectionConfig: req.davincios.collections['davincios-query-presets'].config,
          data: queryPreset,
          req
        }))?.docPermissions;
      }
    } catch (err) {
      req.davincios.logger.error(`Error fetching query preset or preset permissions: ${err}`);
    }
  }
  ...
  const renderedFilters = renderFilters(collectionConfig.fields, req.davincios.importMap);
