 ...
 * When draft=true is used in the query, DaVinciOS returns the latest draft version if it exists.
 ...
 */export async function enrichDocsWithVersionStatus({
  ...
  try {
    ...
    // const { values } = await req.davincios.findDistinctVersions({
    ...
    const publishedVersions = await req.davincios.findVersions({
      collection: collectionConfig.slug,
      depth: 0,
      limit: 0,
      pagination: false,
