...
;
  ...
  if (field.type === 'tabs' && 'tabs' in field) {
    ...
    for (let i = 0; i < maxLength; i++) {
      ...
      const toBlock = req.davincios.blocks[blockSlugToMatch] ?? (field.blockReferences ?? field.blocks).find(block => typeof block !== 'string' && block.slug === blockSlugToMatch);
      let fields = [];
      if (toRow.blockType === fromRow.blockType) {
        fields = toBlock.fields;
      } else {
        const fromBlockSlugToMatch = toRow?.blockType ?? fromRow?.blockType;
        const fromBlock = req.davincios.blocks[fromBlockSlugToMatch] ?? (field.blockReferences ?? field.blocks).find(block => typeof block !== 'string' && block.slug === fromBlockSlugToMatch);
        if (fromBlock) {
