...
export const RootPage = async ({
  ...
  if (collectionConfig && segments.length === 2) {
    if (config.folders && collectionConfig.folders && segments[1] !== config.folders.slug) {
      await getPreferences(`collection-${collectionConfig.slug}`, req.davincios, req.user.id, config.admin.user).then(res => {
        if (res && res.value) {
  ...
  const dbHasUser = req.user || (await req.davincios.db.findOne({
    collection: userSlug,
    req
  })?.then(doc => !!doc));
  /**
  ...
  */
  if (!DefaultView?.Component && !DefaultView?.davinciosComponent) {
    if (req?.user) {
      notFound();
    }
    if (dbHasUser) {
  ...
  if (!DefaultView?.Component && !DefaultView?.davinciosComponent && !dbHasUser) {
    redirect(adminRoute);
  }
  ...
  const RenderedView = RenderServerComponent({
    ...
    Component: DefaultView.davinciosComponent,
    ...
    serverProps: {
      ...
      initPageResult: {
        ...
        docID: routeParams.id,
        globalConfig,
        languageOptions: Object.entries(req.davincios.config.i18n.supportedLanguages || {}).reduce((acc, [language, languageConfig]) => {
          if (Object.keys(req.davincios.config.i18n.supportedLanguages).includes(language)) {
            acc.push({
