import { deepMergeSimple } from '@davincios/translations/utilities';
 ...
 */ export const sanitizeField = async ({ collectionConfig, config, existingFieldNames, field, globalConfig, index, isTopLevelField, joinPath, joins, orderableJoins, parentIndexPath, parentIsLocalized, parentSchemaPath, polymorphicJoins, requireFieldLevelRichTextEditor, richTextSanitizationPromises, validRelationships })=>{
    ...
    if (field.type === 'relationship' || field.type === 'upload') {
        ...
        if (field.min && !field.minRows) {
            console.warn(`(DaVinciOS): The "min" property is deprecated for the Relationship field "${field.name}" and will be removed in a future version. Please use "minRows" instead.`);
            field.minRows = field.min;
        }
        if (field.max && !field.maxRows) {
            console.warn(`(DaVinciOS): The "max" property is deprecated for the Relationship field "${field.name}" and will be removed in a future version. Please use "maxRows" instead.`);
    ...
    if (fieldAffectsData) {
        ...
        if (typeof field.localized !== 'undefined') {
            ...
            if (process.env.NEXT_PUBLIC_DaVinciOS_COMPATIBILITY_allowLocalizedWithinLocalized !== 'true' && parentIsLocalized && // @todo DaVinciOS_DO_NOT_SANITIZE_LOCALIZED_PROPERTY=true will be the default in 4.0
            process.env.DaVinciOS_DO_NOT_SANITIZE_LOCALIZED_PROPERTY !== 'true') {
