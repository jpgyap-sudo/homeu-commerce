...
export const virtualFieldPopulationPromise = async ({ name, draft, fallbackLocale, fields, hasMany, locale, overrideAccess, ref, req, segments, showHiddenFields, siblingDoc })=>{
    ...
    if ((currentField.type === 'relationship' || currentField.type === 'upload') && typeof currentField.relationTo === 'string') {
        ...
        const currentFields = req.DaVinciOS.collections[currentField.relationTo]?.config.flattenedFields;
        ...
        if (currentField.hasMany) {
            ...
            const populatedDocs = await Promise.all(docIDs.map((docID)=>{
                return req.DaVinciOSDataLoader.load(createDataloaderCacheKey({
                    collectionSlug,
                    currentDepth: 0,
                    depth: 0,
                    docID,
            ...
            for (const doc of populatedDocs){
                ...
                await virtualFieldPopulationPromise({
                    ...
                    draft,
                    fallbackLocale,
                    fields: req.DaVinciOS.collections[currentField.relationTo].config.flattenedFields,
                    hasMany: true,
                    locale,
                    overrideAccess,
                    ref: doc,
        ...
        const populatedDoc = await req.DaVinciOSDataLoader.load(createDataloaderCacheKey({
