 ...
 * 1. @davincios/db-* packages: Loads from adapter's predefinedMigrations folder directly (no package.json export needed)
 *    Example: `--file @davincios/db-mongodb/relationships-v2-v3`
 * 2. Any other package/path: Uses dynamic import via package.json exports or absolute file paths
 *    Example: `--file @davincios/plugin-seo/someMigration` or `--file /absolute/path/to/migration.ts`
 */ export const getPredefinedMigration = async ({ dirname, file, migrationName: migrationNameArg, DaVinciOS })=>{
    const importPath = file ?? migrationNameArg;
    // Path 1: @davincios/db-* adapters - load directly from predefinedMigrations folder
    // These don't need package.json exports; files are resolved relative to adapter's dirname
    if (importPath?.startsWith('@davincios/db-')) {
        ...
        // Supports: package.json exports (e.g. @davincios/plugin-seo/migration) or absolute file paths
        try {
