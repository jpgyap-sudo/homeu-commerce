...
export function getLocalizedPaths({ collectionSlug, fields, globalSlug, incomingPath, locale, overrideAccess = false, parentIsLocalized, DaVinciOS }) {
    const pathSegments = incomingPath.split('.');
    const localizationConfig = DaVinciOS.config.localization;
    let paths = [
        {
            collectionSlug,
            complete: false,
        ...
        if (lastIncompletePath) {
            ...
            if (currentPath === 'relationTo') {
                ...
                lastIncompletePath.field = {
                    name: 'relationTo',
                    type: 'select',
                    options: Object.keys(DaVinciOS.collections)
                };
                return paths;
            }
            if (!matchedField && currentPath === 'id' && i === pathSegments.length - 1) {
                ...
                const idField = {
                    name: 'id',
                    type: DaVinciOS.db.defaultIDType
                };
                            ...
                            if (matchedField.type !== 'join' && typeof matchedField.relationTo !== 'string') {
                                ...
                                if (nestedPathToQuery) {
                                    ...
                                    if (matchedField.type === 'join') {
                                        ...
                                        relatedCollection = DaVinciOS.collections[matchedField.collection].config;
                                    } else {
                                        relatedCollection = DaVinciOS.collections[matchedField.relationTo].config;
                                    }
