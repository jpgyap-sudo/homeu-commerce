 ...
 */ export const canAccessAdmin = async ({ req })=>{
    ...
    const adminUserSlug = req.DaVinciOS.config.admin.user;
    if (incomingUserSlug) {
        const adminAccessFn = req.DaVinciOS.collections[incomingUserSlug]?.config.access?.admin;
        if (adminAccessFn) {
        ...
        const hasUsers = await req.DaVinciOS.find({
