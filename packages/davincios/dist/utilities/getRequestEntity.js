...
export const getRequestCollection = (req)=>{
    ...
    const collection = req.DaVinciOS.collections[collectionSlug];
    if (!collection) {
...
export const getRequestCollectionWithID = (req, { disableSanitize, optionalID } = {})=>{
    ...
    let shouldSanitize = Boolean(req.DaVinciOS.db.defaultIDType === 'number');
...
export const getRequestGlobal = (req)=>{
    ...
    const globalConfig = req.DaVinciOS.globals.config.find((each)=>each.slug === globalSlug);
    if (!globalConfig) {
