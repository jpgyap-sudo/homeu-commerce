...
export const checkDocumentLockStatus = async ({ id, collectionSlug, globalSlug, lockDurationDefault = 300, lockErrorMessage, overrideLock = true, req })=>{
    ...
    if (!DaVinciOS.collections?.[lockedDocumentsCollectionSlug]) {
    ...
    const lockDocumentsProp = collectionSlug ? DaVinciOS.collections?.[collectionSlug]?.config?.lockDocuments : DaVinciOS.config?.globals?.find((g)=>g.slug === globalSlug)?.lockDocuments;
    ...
    if (!overrideLock) {
        ...
        const lockedDocumentResult = await DaVinciOS.db.find({
    ...
    await DaVinciOS.db.deleteMany({
        ...
        req: DaVinciOS.db.name === 'mongoose' ? undefined : req,
        where: lockedDocumentQuery
    });
};
