...
import { checkDaVinciOSDependencies } from './checkDaVinciOSDependencies.js';
 ...
 */ export class BaseDaVinciOS {
   ...
   * @param options
   */ async init(options) {
        if (process.env.NODE_ENV !== 'production' && process.env.DaVinciOS_DISABLE_DEPENDENCY_CHECKER !== 'true' && !checkedDependencies) {
            checkedDependencies = true;
            void checkDaVinciOSDependencies();
        }
        ...
        if (this.config.email instanceof Promise) {
            ...
            if (process.env.NEXT_PHASE !== 'phase-production-build') {
                this.logger.warn(`No email adapter provided. Email will be written to console. More info at https://DaVinciOScms.com/docs/email/overview.`);
            }
        ...
        if (process.env.VERCEL) {
            ...
            if (uploadCollWithoutAdapter.length) {
                ...
                this.logger.warn(`Collections with uploads enabled require a storage adapter when deploying to Vercel. Collection(s) without storage adapters: ${slugs}. See https://DaVinciOScms.com/docs/upload/storage-adapters for more info.`);
            }
        }
        /**
     * @todo: Remove in v4.
...
const initialized = new BaseDaVinciOS();
// eslint-disable-next-line no-restricted-exports
export default initialized;
export const reload = async (config, DaVinciOS, skipImportMapGeneration, options)=>{
    if (typeof DaVinciOS.db.destroy === 'function') {
        // Only destroy db, as we then later only call DaVinciOS.db.init and not DaVinciOS.init
        await DaVinciOS.db.destroy();
    }
    DaVinciOS.config = config;
    DaVinciOS.collections = config.collections.reduce((collections, collection)=>{
        collections[collection.slug] = {
            config: collection,
            customIDType: DaVinciOS.collections[collection.slug]?.customIDType
        };
        return collections;
    }, {});
    DaVinciOS.blocks = config.blocks.reduce((blocks, block)=>{
        blocks[block.slug] = block;
        return blocks;
    }, {});
    DaVinciOS.globals = {
    ...
    // TODO: support HMR for other props in the future (see DaVinciOS/src/index init()) that may change on DaVinciOS singleton
    // Generate types
    ...
    if (DaVinciOS.db?.init) {
        await DaVinciOS.db.init();
    }
    if (!options?.disableDBConnect && DaVinciOS.db.connect) {
        await DaVinciOS.db.connect({
            hotReload: true
        });
    }
    ;
    global._DaVinciOS_clientConfigs = {};
    global._DaVinciOS_schemaMap = null;
    global._DaVinciOS_clientSchemaMap = null;
    global._DaVinciOS_doNotCacheClientConfig = true // This will help refreshing the client config cache more reliably. If you remove this, please test HMR + client config refreshing (do new fields appear in the document?)
    ;
    global._DaVinciOS_doNotCacheSchemaMap = true;
    global._DaVinciOS_doNotCacheClientSchemaMap = true;
};
let _cached = global._DaVinciOS;
if (!_cached) {
    _cached = global._DaVinciOS = new Map();
}
/**
 ...
 */ export const getDaVinciOS = async (options)=>{
    if (!options?.config) {
        throw new Error('Error: the DaVinciOS config is required for getDaVinciOS to work.');
    }
    ...
    if (cached.DaVinciOS) {
        if (options.cron && !cached.initializedCrons) {
            // getDaVinciOS called with crons enabled, but existing cached version does not have crons initialized. => Initialize crons in existing cached version
            cached.initializedCrons = true;
            await cached.DaVinciOS._initializeCrons();
        }
        if (cached.reload === true) {
            let resolve;
            // getDaVinciOS is called multiple times, in parallel. However, we only want to run `await reload` once. By immediately setting cached.reload to a promise,
            // we can ensure that all subsequent calls will wait for the first reload to finish. So if we set it here, the 2nd call of getDaVinciOS
            // will reach `if (cached.reload instanceof Promise) {` which then waits for the first reload to finish.
            cached.reload = new Promise((res)=>resolve = res);
            const config = await options.config;
            // Reload the DaVinciOS instance after a config change (triggered by HMR in development).
            // The second parameter (false) forces import map regeneration rather than deciding based on options.importMap.
            //
            // Why we always regenerate import map: getDaVinciOS() may be called from multiple sources (admin panel, frontend, etc.)
            ...
            //
            // Example scenario: If the frontend calls getDaVinciOS() without importMap first, followed by the admin
            ...
            await reload(config, cached.DaVinciOS, false, options);
            resolve();
            cached.reload = false;
        }
        ...
        if (options?.importMap) {
            cached.DaVinciOS.importMap = options.importMap;
        }
        return cached.DaVinciOS;
    }
    try {
        if (!cached.promise) {
            // no need to await options.config here, as it's already awaited in the BaseDaVinciOS.init
            cached.promise = new BaseDaVinciOS().init(options);
        }
        cached.DaVinciOS = await cached.promise;
        if (!cached.ws && process.env.NODE_ENV !== 'production' && process.env.NODE_ENV !== 'test' && process.env.DISABLE_DaVinciOS_HMR !== 'true') {
            try {
                ...
                cached.ws = new WebSocket(process.env.DaVinciOS_HMR_URL_OVERRIDE ?? `${protocol}://localhost:${port}${prefix}${path}`);
                cached.ws.onmessage = (event)=>{
                    if (cached.reload instanceof Promise) {
                        // If there is an in-progress reload in the same getDaVinciOS
...
export { deepMergeSimple } from '@davincios/translations/utilities';
