...
export const loginOperation = async (incomingArgs)=>{
    ...
    const { collection: { config: collectionConfig }, data, depth, overrideAccess = false, req, req: { fallbackLocale, locale, DaVinciOS, DaVinciOS: { secret } }, showHiddenFields } = args;
    // /////////////////////////////////////
    // Login
    // /////////////////////////////////////
    ...
    let user = await DaVinciOS.db.findOne({
        collection: collectionConfig.slug,
        req,
        where: whereConstraint
    });
     ...
     */ if (maxLoginAttemptsEnabled) {
            const { lockUntil, loginAttempts } = await DaVinciOS.db.findOne({
                collection: collectionConfig.slug,
                req,
                select: {
                    lockUntil: true,
