...
export async function getAccessResults({ req }) {
    ...
    const userCollectionConfig = user && user.collection ? DaVinciOS?.collections?.[user.collection]?.config : null;
    if (userCollectionConfig && DaVinciOS.config.admin.user === user?.collection) {
    ...
    await Promise.all(DaVinciOS.config.collections.map(async (collection)=>{
    ...
    await Promise.all(DaVinciOS.config.globals.map(async (global)=>{
