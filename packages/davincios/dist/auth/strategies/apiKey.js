...
export const APIKeyAuthentication = (collectionConfig)=>async ({ headers, isGraphQL = false, DaVinciOS })=>{
        ...
        if (authHeader?.startsWith(`${collectionConfig.slug} API-Key `)) {
            ...
            const sha1APIKeyIndex = crypto.createHmac('sha1', DaVinciOS.secret).update(apiKey).digest('hex');
            const sha256APIKeyIndex = crypto.createHmac('sha256', DaVinciOS.secret).update(apiKey).digest('hex');
            ...
            try {
                ...
                const userQuery = await DaVinciOS.find({
