...
export const incrementLoginAttempts = async ({ collection, DaVinciOS, user })=>{
    ...
    if (user.lockUntil && !isUserLocked(new Date(user.lockUntil))) {
        ...
        const updatedUser = await DaVinciOS.db.updateOne({
            id: user.id,
    ...
    if (reachedMaxAttemptsForNextUser && (!updatedLockUntil || !isUserLocked(new Date(updatedLockUntil)))) {
        ...
        if (collection.auth.useSessions) {
            ...
            const currentUser = await DaVinciOS.db.findOne({
                collection: collection.slug,
                select: {
                    sessions: true
                },
