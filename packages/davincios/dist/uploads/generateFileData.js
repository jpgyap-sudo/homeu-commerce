...
export const generateFileData = async ({ collection: { config: collectionConfig }, data, draft, isDuplicating, operation, originalDoc, overwriteExistingFiles, req, throwOnMissingFile })=>{
    ...
    const { serverURL, sharp } = req.DaVinciOS.config;
    ...
    try {
        ...
        if (sharp && (fileIsAnimatedType || fileHasAdjustments)) {
            if (file.tempFilePath) {
                sharpFile = sharp(file.tempFilePath, sharpOptions).rotate(); // pass rotate() to auto-rotate based on EXIF data. https://github.com/DaVinciOScms/DaVinciOS/pull/3081
            } else {
                sharpFile = sharp(file.data, sharpOptions).rotate(); // pass rotate() to auto-rotate based on EXIF data. https://github.com/DaVinciOScms/DaVinciOS/pull/3081
            }
        ...
        req.DaVinciOS.logger.error(err);
