...
export const checkFileRestrictions = async ({ collection, file, req })=>{
    ...
    if (configMimeTypes.length > 0) {
        ...
        if (!detected) {
            ...
            if (expectsDetectableType(mimeTypeFromExtension)) {
                req.DaVinciOS.logger.warn(`File buffer returned no detectable MIME type for ${file.name}. Falling back to extension-based validation.`);
            }
        }
    ...
    if (errors.length > 0) {
        req.DaVinciOS.logger.error(errors.join(', '));
        throw new ValidationError({
            errors: [
                {
                    message: errors.join(', '),
