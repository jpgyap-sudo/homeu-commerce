...
export async function createImageSizes({ config, dimensions, file, focalPoint, mimeType, req, savedFilename, sharp, staticPath, withMetadata }) {
    ...
    const sharpBase = sharp(file.tempFilePath || file.data, sharpOptions).rotate() // pass rotate() to auto-rotate based on EXIF data. https://github.com/DaVinciOScms/DaVinciOS/pull/3081
    ;
    ...
    await Promise.all(imageSizes.map(async (imageResizeConfig)=>{
        ...
        if (req.DaVinciOSUploadSizes) {
            req.DaVinciOSUploadSizes[imageResizeConfig.name] = bufferData;
        }
