...
export const getJobsLocalAPI = (DaVinciOS)=>({
        handleSchedules: async (args)=>{
            const newReq = args?.req ?? await createLocalReq({}, DaVinciOS);
       ...
       */ const accessFn = DaVinciOS.config.jobs?.access?.queue ?? (()=>true);
            ...
            if (args.queue) {
                ...
                // use that
                const workflow = DaVinciOS.config.jobs?.workflows?.find(({ slug })=>slug === args.workflow);
                if (workflow?.queue) {
            ...
            if (DaVinciOS.config.jobs?.enableConcurrencyControl) {
                ...
                if (args.workflow) {
                    ...
                    const task = DaVinciOS.config.jobs?.tasks?.find(({ slug })=>slug === args.task);
                    if (task?.concurrency) {
                ...
                if (concurrencyKey) {
                    ...
                    if (supersedes) {
                        if (DaVinciOS.config.jobs.runHooks) {
            ...
            if (DaVinciOS?.config?.jobs?.depth || DaVinciOS?.config?.jobs?.runHooks) {
                return await DaVinciOS.create({
                    collection: jobsCollectionSlug,
                    data,
                    depth: DaVinciOS.config.jobs.depth ?? 0,
                    overrideAccess,
                    req
                });
            } else {
       ...
       */ const accessFn = DaVinciOS.config.jobs?.access?.cancel ?? (()=>true);
                const hasAccess = await accessFn({
                    req
                });
                if (!hasAccess) {
