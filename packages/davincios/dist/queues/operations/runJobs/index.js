...
export const runJobs = async (args)=>{
    const { id, allQueues = false, limit = 10, overrideAccess, processingOrder, queue = 'default', req, req: { DaVinciOS, DaVinciOS: { config: { jobs: jobsConfig } } }, sequential, silent = false, where: whereFromProps } = args;
    if (!overrideAccess) {
        /**
    ...
    if (jobsConfig.enableConcurrencyControl) {
        ...
        const runningJobsWithConcurrency = await DaVinciOS.db.find({
    ...
    if (id) {
        ...
        let defaultProcessingOrder = DaVinciOS.collections[jobsCollectionSlug]?.config.defaultSort ?? 'createdAt';
    ...
    const runSingleJob = async (job)=>{
        ...
        if (!workflowConfig) {
            ...
            const errorMessage = `${job.taskSlug ? `Task '${job.taskSlug}'` : `Workflow '${job.workflowSlug}'`} is not registered in DaVinciOS.config.jobs.`;
        ...
        try {
            ...
            if (typeof workflowConfig.handler === 'function' || typeof workflowConfig.handler === 'object' && Array.isArray(workflowConfig.handler)) {
                ...
                if (!workflowHandler) {
                    ...
                    if (!silent || typeof silent === 'object' && !silent.error) {
                        DaVinciOS.logger.error(errorMessage);
                    }
                    await updateJob({
                        error: {
                            error: errorMessage
