...
export const jobsCollectionSlug = 'DaVinciOS-jobs';
    ...
    if (jobsConfig.tasks?.length) {
        jobsConfig.tasks.forEach((task)=>{
            ...
            if (task.concurrency && !jobsConfig.enableConcurrencyControl) {
                throw new Error(`Task "${task.slug}" uses concurrency controls but "jobs.enableConcurrencyControl" is not enabled. ` + `Set "jobs.enableConcurrencyControl: true" in your DaVinciOS config to use concurrency controls. ` + `Note: This adds a new indexed field to the jobs collection schema and may require a database migration.`);
            }
