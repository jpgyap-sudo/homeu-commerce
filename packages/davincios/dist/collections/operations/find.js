...
export const findOperation = async (incomingArgs)=>{
    ...
    try {
        ...
        const req = args.req;
        const includeLockStatus = includeLockStatusFromArgs && req.DaVinciOS.collections?.[lockedDocumentsCollectionSlug];
        const { fallbackLocale, locale, DaVinciOS } = req;
        ...
        if (hasDraftsEnabled(collectionConfig) && draftsEnabled) {
            ...
            result = await DaVinciOS.db.queryDrafts({
                collection: collectionConfig.slug,
                joins: req.DaVinciOSAPI === 'GraphQL' ? false : sanitizedJoins,
            ...
            result = await DaVinciOS.db.find({
                collection: collectionConfig.slug,
                draftsEnabled,
                joins: req.DaVinciOSAPI === 'GraphQL' ? false : sanitizedJoins,
                limit: sanitizedLimit,
        ...
        if (includeLockStatus) {
            try {
                ...
                const lockedDocuments = await DaVinciOS.find({
