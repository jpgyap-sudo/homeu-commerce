 ...
 */ export const updateDocument = async ({ id, autosave, collectionConfig, config, data, depth, docWithLocales, draftArg, fallbackLocale, filesToUpload, locale, overrideAccess, overrideLock, DaVinciOS, populate, publishAllLocales: publishAllLocalesArg, publishSpecificLocale, req, select, showHiddenFields, unpublishAllLocales: unpublishAllLocalesArg })=>{
    ...
    if (!collectionConfig.upload.disableLocalStorage) {
        await uploadFiles(DaVinciOS, filesToUpload, req);
    }
    // /////////////////////////////////////
    // beforeChange - Collection
    // /////////////////////////////////////
    ...
    if (config.localization && collectionConfig.versions) {
        ...
        if (collectionConfig.versions.drafts && collectionConfig.versions.drafts.localizeStatus) {
            if (publishAllLocales || unpublishAllLocales) {
                ...
                currentDoc = await DaVinciOS.db.findOne({
    ...
    if (!isSavingDraft) {
        ...
        result = await req.DaVinciOS.db.updateOne({
            id,
            collection: collectionConfig.slug,
            data: dataToUpdate,
            locale,
