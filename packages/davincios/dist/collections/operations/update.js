...
export const updateOperation = async (incomingArgs)=>{
    ...
    try {
        ...
        const { autosave = false, collection: { config: collectionConfig }, collection, depth, draft: draftArg = false, limit = 0, overrideAccess, overrideLock, overwriteExistingFiles = false, populate, publishAllLocales, publishSpecificLocale, req: { fallbackLocale, locale, DaVinciOS: { config }, DaVinciOS }, req, select: incomingSelect, showHiddenFields, sort: incomingSort, trash = false, unpublishAllLocales, where } = args;
        if (!where) {
        ...
        if (hasDraftsEnabled(collectionConfig) && (shouldSaveDraft || isTrashAttempt)) {
            ...
            await validateQueryPaths({
                ...
                overrideAccess: overrideAccess,
                req,
                versionFields: buildVersionCollectionFields(DaVinciOS.config, collection.config, true),
                where: appendVersionToQueryKey(where)
            });
            const query = await DaVinciOS.db.queryDrafts({
            ...
            const query = await DaVinciOS.db.find({
                collection: collectionConfig.slug,
                limit,
                locale: locale,
                pagination: false,
        ...
        const promises = docs.map(async (docWithLocales)=>{
            ...
            try {
                ...
                if (req.DaVinciOS.db.bulkOperationsSingleTransaction) {
