...
export const restoreVersionOperation = async (args)=>{
    const { id, collection: { config: collectionConfig }, depth, draft: draftArg = false, overrideAccess = false, populate, req, req: { fallbackLocale, locale, DaVinciOS }, select: incomingSelect, showHiddenFields } = args;
    try {
        ...
        // /////////////////////////////////////
        const { docs: versionDocs } = await req.DaVinciOS.db.findVersions({
        ...
        const doc = await req.DaVinciOS.db.findOne(findOneArgs);
        ...
        const validationLocale = DaVinciOS.config.localization ? DaVinciOS.config.localization.defaultLocale : locale;
