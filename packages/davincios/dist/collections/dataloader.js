...
// DaVinciOS uses `dataloader` to solve the classic GraphQL N+1 problem.
...
const batchAndLoadDocs = (req)=>async (keys)=>{
    ...
    **/ const batchByFindArgs = {};
        for (const key of keys){
            ...
            const idType = DaVinciOS.collections?.[collection]?.customIDType || DaVinciOS.db.defaultIDType;
        ...
        for (const [batchKey, ids] of Object.entries(batchByFindArgs)){
            ...
            const enableTrash = Boolean(DaVinciOS.collections?.[collection]?.config?.trash);
...
export const getDataLoader = (req)=>{
    ...
    dataLoader.find = (args)=>{
        ...
        const request = req.DaVinciOS.find(args);
