 ...
 */ export async function resolveImportMapFilePath({ adminRoute = '/admin', importMapFile, rootDir }) {
    ...
    if (importMapFile?.length) {
        ...
        const appLocation = path.resolve(rootDir, `app/(DaVinciOS)${adminRoute}/`);
        const srcAppLocation = path.resolve(rootDir, `src/app/(DaVinciOS)${adminRoute}/`);
        if (appLocation && await pathOrFileExists(appLocation)) {
            ...
            return new Error(`Could not find DaVinciOS import map folder. Looked in ${appLocation} and ${srcAppLocation}`);
        }
    }
