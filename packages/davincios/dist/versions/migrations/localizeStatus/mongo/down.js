...
export async function down(args) {
    const { collectionSlug, globalSlug, DaVinciOS } = args;
    ...
    const entityConfig = collectionSlug ? DaVinciOS.config.collections.find((c)=>c.slug === collectionSlug) : DaVinciOS.config.globals.find((g)=>g.slug === globalSlug);
    if (!entityConfig) {
    ...
    const defaultLocale = DaVinciOS.config.localization.defaultLocale;
