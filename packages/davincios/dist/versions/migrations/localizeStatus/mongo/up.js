...
export async function up(args) {
    const { collectionSlug, globalSlug, DaVinciOS, req } = args;
    ...
    if (!DaVinciOS.config.localization) {
        throw new Error('Localization is not enabled in DaVinciOS config');
    }
    // Check if versions are enabled on this collection/global
    let entityConfig;
    if (collectionSlug) {
        const collection = DaVinciOS.config.collections.find((c)=>c.slug === collectionSlug);
        if (collection) {
            entityConfig = collection;
        }
    } else if (globalSlug) {
        const global = DaVinciOS.config.globals.find((g)=>g.slug === globalSlug);
        if (global) {
            entityConfig = global;
        }
    }
    ...
    const connection = DaVinciOS.db.connection;
    // Get filtered locales if filterAvailableLocales is defined
    let locales = DaVinciOS.config.localization.localeCodes;
    if (typeof DaVinciOS.config.localization.filterAvailableLocales === 'function') {
        const filteredLocaleObjects = await DaVinciOS.config.localization.filterAvailableLocales({
            locales: DaVinciOS.config.localization.locales,
            req
        });
    ...
    const versionLocaleStatus = calculateVersionLocaleStatuses(versionRecords, locales, DaVinciOS);
    DaVinciOS.logger.info({
