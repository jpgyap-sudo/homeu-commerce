import { en } from '@davincios/translations/languages/en';
...
const sanitizeAdminConfig = (configToSanitize)=>{
    ...
    if (configToSanitize?.compatibility?.allowLocalizedWithinLocalized) {
        process.env.NEXT_PUBLIC_DaVinciOS_COMPATIBILITY_allowLocalizedWithinLocalized = 'true';
    }
    ...
    (sanitizedConfig.admin.dashboard ??= {
        ...
        slug: 'collections',
        Component: '@davincios/next/rsc#CollectionCards',
        minWidth: 'full'
    });
...
export const sanitizeConfig = async (incomingConfig)=>{
   ...
   */ config.blocks = [];
    ...
    // @ts-expect-error indexSortableFields is only valid for @davincios/db-mongodb
