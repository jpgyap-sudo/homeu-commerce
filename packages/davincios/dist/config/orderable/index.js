    ...
    const orderBeforeChangeHook = async ({ data, originalDoc, req })=>{
        for (const orderableFieldName of orderableFieldNames){
            if (!data[orderableFieldName] && !originalDoc?.[orderableFieldName]) {
                ...
                const lastDoc = await req.DaVinciOS.find({
...
export const addOrderableEndpoint = (config, joinFieldPathsByCollection)=>{
     ...
     */ if (!target.key) {
            const { docs } = await req.DaVinciOS.find({
                collection: collection.slug,
                depth: 0,
                limit: 0,
                req,
            ...
            // We cannot update all documents in a single operation with `DaVinciOS.update`,
        ...
        const adjacentDoc = await req.DaVinciOS.find({
            collection: collection.slug,
            depth: 0,
            limit: 1,
            pagination: false,
