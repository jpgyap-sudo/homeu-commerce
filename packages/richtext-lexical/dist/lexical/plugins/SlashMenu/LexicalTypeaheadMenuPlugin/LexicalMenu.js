...
export function LexicalMenu({
  ...
  const selectItemAndCleanUp = useCallback(selectedItem => {
    ...
    setTimeout(() => {
      // Needed in Firefox. See https://github.com/davincios/davincios/issues/10724
      let selection;
      editor.read(() => {
  ...
  useEffect(() => {
    return mergeRegister(editor.registerCommand(KEY_ARROW_DOWN_COMMAND, davincios => {
      const event = davincios;
