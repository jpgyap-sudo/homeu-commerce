...
export function initLexicalFeatures(args) {
  ...
  */
  ...
  for (const [featureKey, resolvedFeature] of resolvedFeatureMapArray) {
    ...
    */
    const ClientFeatureDaVinciOSComponent = resolvedFeature.ClientFeature;
    if (ClientFeatureDaVinciOSComponent) {
      const clientFeatureProvider = getFromImportMap({
        importMap: args.davincios.importMap,
        DaVinciOSComponent: ClientFeatureDaVinciOSComponent,
      ...
      if (typeof ClientFeatureDaVinciOSComponent === 'object' && ClientFeatureDaVinciOSComponent.clientProps) {
        clientFeatureProps.clientProps = ClientFeatureDaVinciOSComponent.clientProps;
      }
    ...
    */ // The args.fieldSchemaMap generated before in buildFormState should contain all of lexical features' sub-field schemas
    ...
    if (resolvedFeature.componentImports && typeof resolvedFeature.componentImports === 'object' && !Array.isArray(resolvedFeature.componentImports)) {
      for (const [key, davinciosComponent] of Object.entries(resolvedFeature.componentImports)) {
