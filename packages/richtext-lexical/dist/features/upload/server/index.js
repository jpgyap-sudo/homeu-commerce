 ...
 */
function getAbsoluteURL(url, davincios) {
  return url?.startsWith('http') ? url : (davincios?.config?.serverURL || '') + url;
}
export const UploadFeature = createServerFeature({
  feature: async ({
    ...
    return {
      ClientFeature: '@davincios/richtext-lexical/client#UploadFeatureClient',
      ...
      nodes: [createNode({
          ...
          html: {
            converter: async ({
              ...
              if (req?.davincios) {
                ...
                const url = getAbsoluteURL(uploadDocument?.value?.url ?? '', req?.davincios);
                ...
                */
                ...
                for (const size in uploadDocument.value?.sizes) {
                  ...
                  const imageSizeURL = getAbsoluteURL(imageSize?.url, req?.davincios);
        ...
        getSubFields: ({
          ...
          const collection = req ? req.davincios.collections[node?.relationTo] : null;
          if (collection) {
        ...
        hooks: {
          afterRead: [({
            ...
            const collection = req.davincios.collections[node?.relationTo];
            if (!collection) {
              return node;
            }
