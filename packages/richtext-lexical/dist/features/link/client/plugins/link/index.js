...
export const LinkPlugin = t0 => {
  ...
  if ($[0] !== clientProps.defaultLinkType || $[1] !== clientProps.defaultLinkURL || $[2] !== editor) {
    t1 = () => {
      ...
      return mergeRegister(editor.registerCommand(TOGGLE_LINK_COMMAND, davincios => {
        if (davincios === null) {
          $toggleLink(null);
          return true;
        }
        if (!davincios.fields?.linkType) {
          davincios.fields.linkType = clientProps.defaultLinkType;
        }
        if (!davincios.fields?.url) {
          davincios.fields.url = clientProps.defaultLinkURL;
        }
        $toggleLink(davincios);
        return true;
