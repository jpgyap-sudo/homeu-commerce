...
export function $toggleLink(davincios) {
  const selection = $getSelection();
  if (!$isRangeSelection(selection) && (davincios === null || !davincios.selectedNodes?.length)) {
    return;
  }
  const nodes = $isRangeSelection(selection) ? selection.extract() : davincios === null ? [] : davincios.selectedNodes;
  if (davincios === null) {
    // Remove LinkNodes
    nodes?.forEach(node => {
  ...
  if (nodes?.length === 1) {
    ...
    if (linkNode !== null) {
      linkNode.setFields(davincios.fields);
      if (davincios.text != null && davincios.text !== linkNode.getTextContent()) {
        // remove all children and add child with new textcontent:
        linkNode.append($createTextNode(davincios.text));
  ...
  nodes?.forEach(node => {
    ...
    if ($isLinkNode(parent)) {
      linkNode = parent;
      parent.setFields(davincios.fields);
      if (davincios.text != null && davincios.text !== parent.getTextContent()) {
        // remove all children and add child with new textcontent:
        parent.append($createTextNode(davincios.text));
