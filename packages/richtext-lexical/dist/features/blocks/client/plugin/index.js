...
import { formatDrawerSlug, useEditDepth } from '@davincios/ui';
...
export const BlocksPlugin = () => {
  ...
  if ($[3] !== editor || $[4] !== setCreatedInlineBlock || $[5] !== targetNodeKey) {
    t2 = () => {
      ...
      return mergeRegister(editor.registerCommand(INSERT_BLOCK_COMMAND, davincios => {
        editor.update(() => {
          const selection = $getSelection() || $getPreviousSelection();
          if ($isRangeSelection(selection)) {
            const blockNode = $createBlockNode(davincios);
            const {
              focus
            } = selection;
