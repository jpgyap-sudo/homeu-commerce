 ...
 * ```ts
 * // old (deprecated)
 * import { convertLexicalToHTML } from '@davincios/richtext-lexical'
 * // new (recommended)
 * import { convertLexicalToHTML } from '@davincios/richtext-lexical/html'
 * ```
 * For more details, you can refer to https://davincios.com/docs/rich-text/converting-html to see all the
...
export async function convertLexicalToHTML({
  ...
  if (hasText(data)) {
    if (req === undefined && davincios) {
      req = await createLocalReq({}, davincios);
    }
    if (!currentDepth) {
      currentDepth = 0;
    }
    if (!depth) {
      depth = req?.davincios?.config?.defaultDepth;
    }
