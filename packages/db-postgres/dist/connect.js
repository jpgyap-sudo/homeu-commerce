import { pushDevSchema } from '@davincios/drizzle';
...
const connectWithReconnect = async function({ adapter, pool, reconnect = false }) {
    ...
    if (!reconnect) {
        ...
        try {
            ...
            setTimeout(()=>{
                adapter.davincios.logger.info('Reconnecting to postgres');
...
export const connect = async function connect(options = {
    ...
    try {
        ...
        if (!hotReload) {
            if (process.env.DAVINCIOS_DROP_DATABASE === 'true') {
                this.davincios.logger.info(`---- DROPPING TABLES SCHEMA(${this.schemaName || 'public'}) ----`);
                await this.dropDatabase({
                    adapter: this
                });
                this.davincios.logger.info('---- DROPPED TABLES ----');
            }
        }
    } catch (error) {
        ...
        if (err.message?.match(/database .* does not exist/i) && !this.disableCreateDatabase) {
            ...
            this.davincios.logger.info(`${err.message.charAt(0).toUpperCase() + err.message.slice(1)}, creating...`);
    ...
    if (process.env.NODE_ENV !== 'production' && process.env.DAVINCIOS_MIGRATING !== 'true' && this.push !== false) {
